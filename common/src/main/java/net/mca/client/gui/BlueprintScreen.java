...
public class BlueprintScreen extends ExtendedScreen {
    ...
    public void render(MatrixStack transform, int sizeX, int sizeY, float offset) {
        ...
        switch (page) {
            ...
            case "catalog" -> renderCatalog(transform);
            case "villagers" -> renderVillagers(transform);
            case "rules" -> renderRules(transform);
        }
    ...
    private void renderCatalog(MatrixStack transform) {
        ...
        if (selectedBuilding != null) {
            ...
            for (Map.Entry<Identifier, Integer> b : selectedBuilding.getGroups().entrySet()) {
                textRenderer.drawWithShadow(transform, Text.literal(b.getValue() + " x ").append(getBlockName(b.getKey())), x, y, 0xffffffff);
                y += 10;
            }
        } else {
            //help
    ...
    private void renderRules(MatrixStack transform) {
        ...
        //taxes
        drawCenteredTextWithShadow(transform, textRenderer, Text.translatable("gui.blueprint.taxes"), width / 2, height / 2 + POSITION_TAXES, 0xffffffff);
        if (!rank.isAtLeast(Rank.MERCHANT)) {
            drawCenteredTextWithShadow(transform, textRenderer, Text.translatable("gui.blueprint.rankTooLow"), width / 2, height / 2 + POSITION_TAXES + 15, 0xffffffff);
            toggleButtons(buttonTaxes, false);
        } else {
        ...
        //births
        drawCenteredTextWithShadow(transform, textRenderer, Text.translatable("gui.blueprint.birth"), width / 2, height / 2 + POSITION_BIRTH, 0xffffffff);
        if (!rank.isAtLeast(Rank.NOBLE)) {
            drawCenteredTextWithShadow(transform, textRenderer, Text.translatable("gui.blueprint.rankTooLow"), width / 2, height / 2 + POSITION_BIRTH + 15, 0xffffffff);
            toggleButtons(buttonBirths, false);
        } else {
        ...
        //marriages
        drawCenteredTextWithShadow(transform, textRenderer, Text.translatable("gui.blueprint.marriage"), width / 2, height / 2 + POSITION_MARRIAGE, 0xffffffff);
        if (!rank.isAtLeast(Rank.MAYOR)) {
            drawCenteredTextWithShadow(transform, textRenderer, Text.translatable("gui.blueprint.rankTooLow"), width / 2, height / 2 + POSITION_MARRIAGE + 15, 0xffffffff);
            toggleButtons(buttonMarriage, false);
        } else {
