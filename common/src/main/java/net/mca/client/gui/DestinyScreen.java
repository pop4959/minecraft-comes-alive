...
public class DestinyScreen extends VillagerEditorScreen {
    ...
    protected void setPage(String page) {
        if (page.equals("destiny") && !allowTeleportation) {
            ...
            if (Config.getInstance().destinySpawnLocations.size() == 1) {
                selectStory(Config.getInstance().destinySpawnLocations.get(0));
                return;
            }
        }
        ...
        switch (page) {
            ...
            case "destiny" -> {
                int x = 0;
                int y = 0;
                for (String location : Config.getInstance().destinySpawnLocations) {
                    int rows = (int)Math.ceil(Config.getInstance().destinySpawnLocations.size() / 3.0f);
                    float offsetX = (y + 1) == rows ? (2 - (Config.getInstance().destinySpawnLocations.size() - 1) % 3) / 2.0f : 0;
                    float offsetY = Math.max(0, 3 - rows) / 2.0f;
                    addDrawableChild(new ButtonWidget((int)(width / 2 - 96 * 1.5f + (x + offsetX) * 96), (int)(height / 2 + (y + offsetY) * 20 - 16), 96, 20, new TranslatableText("gui.destiny." + new Identifier(location).getPath()), sender -> {
    ...
    private void selectStory(String location) {
        ...
        story.add(new TranslatableText("destiny.story." + new Identifier(location).getPath()));
